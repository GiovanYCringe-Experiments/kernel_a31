...
ZIPNAME="Lemonade-a31x-$(date '+%Y%m%d-%H%M').zip"
...
make -j$(nproc --all) O=out ARCH=arm64 CC=clang CROSS_COMPILE=aarch64-linux-gnu- CLANG_TRIPLE=aarch64-linux-gnu-
...
if [ -f "$kernel" ]; then
	...
	elif ! git clone -q https://github.com/GiovanYCringe/AnyKernel3 -b a31; then
